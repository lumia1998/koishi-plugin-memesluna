...
在当前 `koishi-plugin-memesluna` 中，没有发现“按出现频次自动保存高频图片”的逻辑。已有自动保存主要来自两个明确入口：

1. 控制台上传到某个表情包。
2. 命令偷表情并指定目标表情包。

本次新增的暂缓区也不会按“高频出现”自动抓取图片；它只接收被上传过滤器或偷表情命令识别为不适合直接入库的候选图片。高频统计、自动抓取和阈值入库如果要做，需要额外设计消息监听、图片指纹/哈希、频次表和阈值策略。
